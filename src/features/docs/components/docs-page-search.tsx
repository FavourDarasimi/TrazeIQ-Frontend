...
/**
 * Local fuzzy cmd+K palette over page titles + headings. No external
 * service — the index is built at build time from MDX frontmatter.
 */
export function DocsPageSearch({ entries }: { entries: SearchEntry[] }) {
  ...
  // Skeleton shimmer on query change (local filter is sync; paint never
  // flashes an empty list). Mounted fresh each open, so `false` initial
  // state is the reset — no sync setState in effect.
  useEffect(() => {
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return entries.filter((e) => e.kind === "page").slice(0, 12);
    return entries
      .filter(
        (e) =>
          ...
          <div className="relative flex w-full max-w-[560px] flex-col overflow-hidden rounded-xl border border-line bg-bg-panel shadow-[0_20px_60px_rgba(0,0,0,0.8)]">
            <div className="flex items-center gap-3 border-b border-line px-4 py-3">
              ...
              <input
                ...
                placeholder="Search docs…  e.g. ingestion, OTP, payload"
